...
const mapResponseSchema = z.object({
  skillMatrix: z.array(
    z.object({
      ...
      jdWeight: z.number().min(0).max(10).transform((v) => Math.max(1, Math.min(10, Math.round(v) || 1))),
      jdRequiredLevel: z.number().min(0).max(10).transform((v) => Math.max(1, Math.min(10, Math.round(v) || 1))),
